...
/* Возвращает массив выделенных файлов/папок */
	form._getSelectedValues=function(onlyFiles){
		var ar=[],
			d=this['dirs[]'],
			f=this['files[]'];
		if(d && !onlyFiles){
...
/* button attach */
	todo.get('button_attach').onclick=function(){
		var callback=tinyMCEPopup.getWindowArg("callback"),
			ar = form._getSelectedValues(true),
			limit = tinyMCE.activeEditor.windowManager.params.limit,
			list = tinyMCE.activeEditor.windowManager.params.list;
		for(var i in ar){
			if(list.length < limit){
				var url = '<?=$dir->getAbsPath()?>'+ar[i];
				if(typeof callback == 'function'){
					callback(url);
				}else{
					var win=tinyMCEPopup.getWindowArg("window");
				...
				alert('Превышено допустимое количество прикрепляемых изображений');
				break;
			}
		}
		tinyMCEPopup.close();
...
?>
	...
	<div id="log">
		<div class="buttons">
			<input id="button_attach" type="button" value="Прикрепить" class="add">

...
function deleteRow($id){
	...
	if($mysql->query("delete from `$table` where `$idcol` in (".implode(',',$id).")")){ //удаляем
		if($sortcontrol
			&& ($row_set = $mysql->query("select * from `$table`".($cond ? ' where '.$cond : null)." order by `$sortcol` $sortdirect"))
			...
			if(count($ar))
				return $mysql->query("INSERT INTO `$table` (`$idcol`,`$sortcol`) VALUES ".implode(',',$ar)." ON DUPLICATE KEY UPDATE `$sortcol`=VALUES(`$sortcol`)");
		}
		return true;
	}
}
function moveRow($id,$pos){
	...
	if(!($rs = $mysql->query("select count(*) as `numRows` from `$table`".($cond ? " where $cond" : null)))
	...
	if(!($rs = $mysql->query("select * from `$table`".($cond ? " where $cond" : null)." order by `$sortcol` $sortdirect limit ".($pos-1).",1"))
		|| !($row = mysql_fetch_array($rs))
	) return;
	$target_sort = $row[$sortcol];
	//Корректируем значения поля sort
	if(!$mysql->query("update `$table` set `$sortcol`=`$sortcol` - 1 where".($cond ? ' '.$cond.' and' : null)." `$sortcol` > $source_sort")) return;
	if(!$mysql->query("update `$table` set `$sortcol`=`$sortcol` + 1 where".($cond ? ' '.$cond.' and' : null)." `$sortcol` >= $target_sort")) return;

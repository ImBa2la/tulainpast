...
function __construct($val = null,$name = null,$cache = true){
	...
	if(is_object($val)){
		...
		if(strtolower(substr(trim($val),0,5))=='<?xml'){
			...
			if(file_exists($path)){
				...
				$this->dd = xml::getCache($path);
				if($_xmlDebug) echo '6.'.$this->dd->documentURI.'<br/>';
			}
		}
	}
	if(!$this->dd){
		...
		if($_xmlDebug) echo '7.'.$this->dd->documentURI.'<br/>';
	}
	if(!$this->de() && $name)
	...
	if($cache && $this->documentURI()){
		...
		if($_xmlDebug) echo '<p>'.$this->evaluate('count(//*)').'</p>';
	}
}
...
static function setCache($val){
	global $_xmlCache,$_xmlDebug;
	$dd = null;
	if($val instanceof DOMDocument) $dd = $val;
	elseif($val instanceof xml) $dd = $val->dd();
	if($dd && ($uri = xml::normalizePath($dd->documentURI))){
		$_xmlCache[$uri] = $dd;
		if($_xmlDebug){
			?><pre><? print_r($_xmlCache) ?></pre><?
		}
	}
}
...
static function getElementText($e){
	$text = '';
...
static function setElementText($e,$text){
	if($e->hasChildNodes()) foreach($e->childNodes as $node){
		if('DOMText'!=get_class($node)) continue;
		$e->removeChild($node);
	}

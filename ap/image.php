...
if(substr($filename,0,1)=='/') $filename = $_SERVER['DOCUMENT_ROOT'].$filename;

if(!$filename || !file_exists($filename)) die('file not found');
...
if(isset($_GET['watermark']) 
	&& ($water = urldecode($_GET['watermark']))
){
	if(substr($water,0,1)=='/') $water = $_SERVER['DOCUMENT_ROOT'].$water;
	if(!$water || !file_exists($water)) die('file watermark not found');
	
	$offset = 5;//отступ от правого нижнего края
	$r = imagecreatefrompng($water);
    $x = imagesx($r);
    $y = imagesy($r);
	
    imageAlphaBlending($image_p,1);
    imageAlphaBlending($r,1);
    imagesavealpha($image_p,1);
    imagesavealpha($r,1);
    imagecopyresampled($image_p,$r,$offset,$offset,0,0,$x,$y,$x,$y);
    /*switch (strtolower(pathinfo($filename,PATHINFO_EXTENSION))) {
		case "jpg":imagejpeg($image_p,null,90);break;
		case "jpeg":imagejpeg($image_p,null,90);break;
